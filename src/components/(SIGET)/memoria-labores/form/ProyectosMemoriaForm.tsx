...
const formTextareaClass =
  "w-full rounded-lg border border-border bg-zinc-50 px-3 py-2 text-sm text-foreground outline-none transition-all placeholder:text-muted-foreground/70 focus-visible:ring-2 focus-visible:ring-celeste-trifinio resize-y dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder:text-zinc-500";
...
const Field = ({
  ...
  <textarea {...props} className={cn(formTextareaClass, className)} />
);
...
function ProyectoSubseccion({
  ...
  return (
    <div className={proyectoSeccionShell}>
      <p
        className={cn(
          "flex items-center gap-2 text-sm font-bold mb-4 pb-3 border-b border-border dark:border-zinc-700",
    ...
    <section className={formSectionClass}>
      ...
      <div className="p-4 space-y-5">
        {items.map((item, i) => (
          <div
            key={i}
            className="rounded-xl border border-border dark:border-zinc-700 p-4 space-y-4 bg-muted/20 dark:bg-zinc-950/50"
          >
  ...
  return (
    ...
    >
      {items.map((avance, j) => (
          <div
            key={j}
            className="flex items-start gap-2 rounded-lg border border-border/80 bg-muted/30 p-3 dark:border-zinc-800 dark:bg-zinc-950/60"
          >
            <div className="flex h-10 w-8 shrink-0 items-center justify-center rounded-lg bg-muted/60 text-sm font-black text-muted-foreground dark:bg-zinc-800 dark:text-zinc-300">
              {j + 1}
            </div>
            <div className="flex-1 min-w-0 space-y-2">
              ...
              <div className="grid grid-cols-[1fr_auto_1fr_auto] items-end gap-2">
                ...
                <span className="pb-2.5 text-lg font-black text-muted-foreground dark:text-zinc-500">
                  /
                </span>
                <div className="grid gap-1.5">
                  <Label className="normal-case tracking-normal text-[10px]">
                ...
                <div className="shrink-0 rounded-lg bg-amber-500/10 px-3 h-10 flex items-center justify-center text-sm font-black text-amber-600 dark:text-amber-400 min-w-[4rem]">
                  {avance.meta > 0
              ...
              <div className="space-y-1 pt-1">
                <FieldHint>
                  ...
                  capacitadas, kilómetros de camino).
                </FieldHint>
                <FieldHint>
                  <span className="font-semibold text-foreground/70 dark:text-zinc-400">
                    Logrado:
                ...
                <FieldHint>
                  ...
                  usted defina (10, 50, 100…). La escala no está limitada a 10.
                </FieldHint>
              </div>
            </div>
            <button
              type="button"
              onClick={() => onRemove(j)}
              className="h-10 w-10 shrink-0 flex items-center justify-center rounded-lg border border-border text-muted-foreground hover:bg-red-50 hover:text-red-600 dark:border-zinc-700 dark:text-zinc-400 dark:hover:bg-red-950/40 dark:hover:text-red-400 transition-colors cursor-pointer"
        ...
        />
      <FieldHint>
        Indicadores medibles del avance. Usted define la escala: puede ser 7 de
        10, 15 de 20, 80 de 100, etc.
      </FieldHint>
    </ProyectoSubseccion>
  );
}
...
function ProyectoListaInterna({
  ...
  return (
    <ProyectoSubseccion seccion={addColor} title={titulo} icon={icon}>
      {items.map((value, j) => (
          <div
            key={j}
      ...
      {ayuda ? <FieldHint>{ayuda}</FieldHint> : null}
    </ProyectoSubseccion>
  );
}
